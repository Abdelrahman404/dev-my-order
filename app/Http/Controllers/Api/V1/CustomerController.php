...
use App\Http\Resources\HighlightsApiResource;
...
class CustomerController extends Controller
{
    ...
    public function info(Request $request)
    {
        ...
        $data['user_adressess'] = $user->addresses;
        $data['member_since_days'] =(integer)$request->user()->created_at->diffInDays();
        $data['highlights'] = HighlightsApiResource::collection($user->highlights);
        $data['stories'] = StoriesApiResource::collection($user->stories->take(50));
        $data['posts'] = PostsApiResource::collection($user->posts->load('likes')->take(50));
        $data['posts_counts'] = $user->posts->count();
        $data['followers_count'] = $user->followersCount();
        $data['following_count'] = $user->followingCount();
        unset($data['orders']);
    ...
    public function update_profile(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'password.min' => 'Password must be minimum 6 chracters',
            'password.max' => 'Password must be maxiumum 30 chracters',
        ]);
